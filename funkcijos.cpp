...
void skaiciavimas( std::deque <int> nrez, double& galutinis, int& nsuma, int n, int pasirinkimas, int& r)
{
    studentai a;
    double vidurkis;
    vidurkis=nsuma/n;
...
std::cout<<"failo is "<<q<<" irasu sukurimo laikas:"<<diff.count()<<std::endl;
    ...
    ne_liudeseliai.close();
     ...
     diff = stop-start;
    std::cout<<q<<" irasu nuskaitymo, rezultatu suskaiciavimo, surusiavimo ir isvedimo laikas:"<<diff.count()<<std::endl;
}
